...
class TriggersTests {
    ...
    fun triggers() {
        ...
        val semExp = textToSemanticExpression("Avance de 30 centimètres", textProcessingContext, SemanticSourceEnum.UNKNOWN,
            semanticMemory, linguisticDb)
        val testExecutor = TestExecutor()
        reactFromTrigger(semExp, Locale.FRENCH, semanticMemory, linguisticDb, testExecutor)
        assertEquals("onResource(mission, avance-id, {distance=0,3 mètre})", testExecutor.str)
    }
